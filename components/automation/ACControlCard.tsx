...
export default function ACControlCard({ appliance, onStatusChange, onACTemperatureChange, onModeChange }: ACControlCardProps) {
  const { state: firebaseRelayState, setRelayState } = useRelayControl(appliance.roomId || "unknown", "ac");
  ...
  const handleTempChange = (increment: boolean) => {
    const newTemp = increment ? Math.min(temperature + 1, 30) : Math.max(temperature - 1, 16);
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <span className={`text-3xl ${appliance.status === "on" ? "text-cyan-500" : "text-gray-500"}`}>❄️</span>
          <div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={handleToggle} disabled={isControlling || appliance.controlMode === "auto" || !appliance.isOnline} className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed ${appliance.status === "on" ? "bg-cyan-500" : "bg-gray-300"}`} title={!appliance.isOnline ? "Device is offline" : appliance.controlMode === "auto" ? "Controlled by ML model in Auto mode" : ""}>
      ...
      {appliance.status === "on" && (
        <div className="border-t border-gray-100 pt-4">
          ...
          <div className="flex items-center justify-center gap-4">
            <button onClick={() => handleTempChange(false)} disabled={temperature <= 16 || appliance.controlMode === "auto" || !appliance.isOnline} className="w-10 h-10 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">−</button>
            <div className="text-center">
              <div className="text-3xl font-bold text-cyan-600">{temperature}°</div>
              <div className="text-xs text-gray-500 mt-1">Celsius</div>
            </div>
            <button onClick={() => handleTempChange(true)} disabled={temperature >= 30 || appliance.controlMode === "auto" || !appliance.isOnline} className="w-10 h-10 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">+</button>
          </div>
