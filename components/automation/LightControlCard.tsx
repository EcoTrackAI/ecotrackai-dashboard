...
export const LightControlCard: React.FC<LightControlCardProps> = ({ appliance, onStatusChange, onModeChange }) => {
  const { state: firebaseRelayState, setRelayState } = useRelayControl(appliance.roomId || "unknown", "light");
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <span className={`text-3xl ${appliance.status === "on" ? "text-yellow-500" : "text-gray-500"}`}>💡</span>
          <div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleToggle} disabled={isControlling || appliance.controlMode === "auto" || !appliance.isOnline} className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed ${appliance.status === "on" ? "bg-yellow-500" : "bg-gray-300"}`} title={!appliance.isOnline ? "Device is offline" : appliance.controlMode === "auto" ? "Controlled by ML model in Auto mode" : ""}>
